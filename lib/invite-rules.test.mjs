...
import { decideClaim, CAP, JUDGE_CAP, capFor } from "./invite-rules.ts";
...
test("a judge code serves thirty, a normal handle three", () => {
  assert.equal(capFor("shipaton"), JUDGE_CAP);
  assert.equal(capFor("matt"), CAP);
  assert.deepEqual(decideClaim({ code: { rc: "j" }, friendRc: "f5", alreadyClaimed: null, claims: 5, cap: capFor("shipaton") }), { ok: true, joinN: 6 });
  assert.deepEqual(decideClaim({ code: { rc: "j" }, friendRc: "f31", alreadyClaimed: null, claims: JUDGE_CAP, cap: capFor("shipaton") }), { ok: false, reason: "cap" });
});
