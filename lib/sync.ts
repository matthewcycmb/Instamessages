import { getConversations, getProfile, IgConversation, IgMessage } from "./instagram";
import { supabaseAdmin, Account } from "./supabase";

const CONCURRENCY = 8;

/**
 * Full sync: conversation list + the 20 most recent messages per thread (a
 * hard Instagram API cap), then a profile backfill for threads still missing
 * a name/photo. Used by the sync button and fired in the background right
 * after connect, so the inbox fills without an onboarding step.
 */
...
export async function backfillProfiles(account: Account): Promise<number> {
  const db = supabaseAdmin();
